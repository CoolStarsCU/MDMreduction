...
def correct_bias(argv='to_subtract.lis', suffix='b', overwrite=False):
    ...
    for fl in files:
        ...
        biassecs = ['[1:' + format(overx) + ',1:' + format(n2/2) + ']',
                    '[1:' + format(overx) + ','
                        + format(n2/2+1) + ':' + format(n2) + ']',
                    '[' + format(n1-overx+1) + ':' + format(n1)
                        + ',1:' + format(n2/2) + ']',
                    '[' + format(n1-overx+1) + ':' + format(n1) + ','
                        + format(n2/2+1) + ':' + format(n2) + ']']
        trimsecs = ['[' + format(overx+1) + ':' + format(n1/2)
                    + ',1:' + format(n2/2) + ']',
                    '[' + format(overx+1) + ':' + format(n1/2) + ','
                        + format(n2/2+1) + ':' + format(n2) + ']',
                    '[' + format(n1/2+1) + ':' + format(n1-overx)
                        + ',1:' + format(n2/2) + ']',
                    '[' + format(n1/2+1) + ':' + format(n1-overx) + ','
                        + format(n2/2+1) + ':' + format(n2) + ']']
        finalsecs = ['[1:' + format(n1/2-overx) + ',1:' + format(n2/2) + ']',
                     '[1:' + format(n1/2-overx) + ','
                         + format(n2/2+1) + ':' + format(n2) + ']',
                     '[' + format(n1/2-overx+1) + ':' + format(n1-overx*2)
                         + ',1:' + format(n2/2) + ']',
                     '[' + format(n1/2-overx+1) + ':' + format(n1-overx*2) + ','
                         + format(n2/2+1) + ':' + format(n2) + ']']

        # Subtract bias from each of the four chips separately
        iraf.noao.imred.bias.colbias.median = 'no'
        iraf.noao.imred.bias.colbias.interac = 'no'
        iraf.noao.imred.bias.colbias.function = 'legendre'
        iraf.noao.imred.bias.colbias.order = 4
        for iq in range(4):
        ...
        copyimg = lsobj + '.fits[' + format(overx+1) + ':' + format(n1-overx) \
                  + ',1:' + format(n2) + ']'
        iraf.imarith(operand1 = copyimg, operand2 = 1.0, op = '*',
                     result = lsobj + suffix + '.fits')

        iraf.imcopy.ver = 'no'
        for iq in range(4):
            inputrng = '[1:' + format(n1/2-overx) + ',1:' + format(n2/2) + ']'
            iraf.imcopy(input = lsobj + '.' + format(iq+1) + '.fits' + inputrng,
                        output = lsobj + suffix + '.fits' + finalsecs[iq])
